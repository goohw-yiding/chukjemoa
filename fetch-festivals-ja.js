// 일문 축제(JpnService2 searchFestival2 + detailCommon2) → data/festivals_ja.json
...
async function main(){
  ...
  const fpath=path.join(__dirname,'data','festivals_ja.json'); const cache={};
  ...
  console.log('일문 축제 저장:', out.length, '| 개요:', out.filter(f=>f.ov).length, '| 지역:', JSON.stringify(byr));
}
main();
