...
(function(){
  ...
  function draw(el){
    ...
    try{
      // ⚠️ 지도는 컨테이너에 «덧붙는다» — 안내 문구를 먼저 지우지 않으면 지도 밑에 그대로 남는다.
    ...
    }catch(e){ el.style.display='flex'; el.innerHTML='<span class="nmap-ph">${L.off}</span>'; }
  }
  function load(){
  ...
  // ⚠️ IntersectionObserver 하나에만 기대면, 그게 안 먹는 환경에서 «불러오는 중…»이 영원히 남는다.
  //    (2026-09-02 실측: 어떤 브라우저 창에서는 IO 가 아예 발화하지 않았다.)
  //    그래서 세 갈래로 건다 — ①IO ②스크롤할 때 위치 직접 계산 ③사용자가 눌렀을 때.
  //    전부 «보일 때만» 부르므로 지연 로딩의 이점(비용·속도)은 그대로다.
  function near(el){
  ...
  function sweep(){
    ...
    els.forEach(function(el){
      if(el.dataset.q||el.dataset.done) return;
      if(near(el)){ el.dataset.q='1'; queue.push(el); load(); } else left=true;
    });
    if(!left){ window.removeEventListener('scroll',onScroll); window.removeEventListener('resize',onScroll); }
  }
  var tick=null;
  function onScroll(){ if(tick) return; tick=setTimeout(function(){tick=null;sweep();},150); }
  window.addEventListener('scroll',onScroll,{passive:true});
  window.addEventListener('resize',onScroll,{passive:true});
  els.forEach(function(el){
    el.style.cursor='pointer';
    el.addEventListener('click',function(){ if(!el.dataset.done){ el.dataset.q='1'; queue.push(el); load(); } });
  });
  if('IntersectionObserver' in window){
    var io=new IntersectionObserver(function(ent){
      ent.forEach(function(e){
        if(!e.isIntersecting||e.target.dataset.q) return;
        io.unobserve(e.target); e.target.dataset.q='1'; queue.push(e.target); load();
      });
    },{rootMargin:'200px'});
    els.forEach(function(el){io.observe(el);});
  }
  sweep();                       // 첫 화면에 이미 보이는 지도 처리
  setTimeout(sweep,1500);        // IO 가 조용한 환경 대비 — 한 번 더 확인한다
})();
</script>`;
}
