...
<script>
// ⚠️ intl-fest-extra.js 도 «똑같은» 핸들러를 심는다 — 같은 플래그로 «한 번만» 걸리게 한다
//    (두 번 걸리면 클릭 한 번에 클립보드를 두 번 쓴다). 고칠 땐 두 파일을 같이 고칠 것.
// ⚠️ 2026-09-09 실제로 눌러 보고 잡았다: 클립보드 권한이 denied 면 writeText 가 «조용히 거부»되고
//    실패 콜백이 비어 있어 버튼이 죽은 것처럼 보였다(라벨도 안 바뀐다).
//    거부되면 execCommand 로 한 번 더, 그것도 안 되면 주소를 «선택 상태»로 만들어 손으로 복사하게 한다.
...
document.addEventListener('click',function(e){
  ...
  var done=function(){var o=b.getAttribute('data-lb')||b.textContent;
    b.setAttribute('data-lb',o);b.textContent=b.getAttribute('data-done')||'OK';
    setTimeout(function(){b.textContent=o;},1400);};
  var pick=function(){var box=b.closest('.xcopy'),v=box&&box.querySelector('.vl');
    if(!v) return;var r=document.createRange();r.selectNodeContents(v);
    var s=window.getSelection();s.removeAllRanges();s.addRange(r);
    v.style.background='#ffe9a8';setTimeout(function(){v.style.background='';},1800);};
  var legacy=function(){var ta=document.createElement('textarea');ta.value=t;
    ...
    var okk=false;try{okk=document.execCommand('copy');}catch(_){}
    document.body.removeChild(ta);if(okk){done();}else{pick();}};
