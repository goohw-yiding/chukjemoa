...
const LANG_DATA = { en: apiFestsEn, ja: apiFestsJa, es: apiFestsEs };
function homeAlts() { const a = [{ hreflang: 'ko', href: '/' }]; ['en','ja','es'].forEach(l => { if (LANG_DATA[l].length) a.push({ hreflang: l, href: '/' + l + '/' }); }); a.push({ hreflang: 'x-default', href: '/' }); return a; }
function searchAlts() { const a = [{ hreflang: 'ko', href: '/search/' }]; ['en','ja','es'].forEach(l => { if (LANG_DATA[l].length) a.push({ hreflang: l, href: '/' + l + '/search/' }); }); a.push({ hreflang: 'x-default', href: '/search/' }); return a; }
...
function layout(title, desc, urlPath, content, opts) {
  ...
  const nav = lang === 'ko'
    ? `<nav><a href="/2026-07/">월별 축제</a><a href="/search/">🔎 축제 검색</a><a href="/pet/">🐶 반려견 여행지</a><a href="/accessible/">♿ 무장애 여행</a><a href="/trails/">🥾 걷기 여행</a><a href="/jangteo/">전국 오일장</a><a href="/test/">🔮 취향 테스트</a><a href="/blog/">축제 가이드</a><a href="/en/">EN</a><a href="/ja/">日本語</a><a href="/es/">ES</a></nav>`
    : NAVS[lang];
  const FOOTERS = {
...
// ---------- sitemap / robots ----------
const urls = ['/', ...MONTHS.map(m => `/${m.key}/`), '/search/', '/pet/', ...(apiAccessible.length ? ['/accessible/'] : []), ...(apiTrails.length ? ['/trails/'] : []), '/jangteo/', '/test/', '/blog/', ...posts.map(p => `/blog/${p.slug}/`), '/privacy/', ...(apiFestsEn.length ? ['/en/', '/en/search/'] : []), ...(apiFestsJa.length ? ['/ja/', '/ja/search/'] : []), ...(apiFestsEs.length ? ['/es/', '/es/search/'] : [])];
