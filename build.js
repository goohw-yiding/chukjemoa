...
const MONTHS = [
  ...
  // ⚠️ 1월은 «빠진 게 아니라 여기에 합쳐져 있다»(months: [12, 1]). 그런데 카드 라벨이 「12월·겨울」이라
  //    사용자 눈에는 1월이 없어 보였다(장남 님 지적, 2026-08-31) → 라벨에 1월을 드러낸다.
  //    분리하지 않는 이유: 네이버 실측 검색량이 «1월축제 60/월»뿐이다(12월축제 280·겨울축제 870).
  //    얇은 페이지를 하나 더 만드는 것보다 겨울 한 장에 모으는 편이 낫다.
  { key: '2026-12', months: [12, 1], label: '2026년 12월~2027년 1월 겨울', short: '12·1월 겨울', emoji: '⛄' },
...
const COUPANG = {
  ...
  items: {
    ...
    // 🏛 2026-09-15 — 깔창과 «같은 상품»인데 문구만 실내용이다(psbase/psbases, suncap/suncap98 과 같은 패턴).
    //    박물관·미술관 페이지에 「장터는 오래 걷습니다」를 붙일 수는 없다.
    //    왜 깔창인가: 전시 관람은 두세 시간을 서서 걷는다 — 실내에서 실제로 생기는 불편이 이것이다.
    //    자사 상품(스툴·돗자리·파라솔)은 실내에서 쓸 수 없어 여기엔 맞는 것이 없다.
    indoor_walk: { ico: '👟', t: '전시는 두세 시간 서서 봅니다', s: '5레이어 쿠션 깔창 · 양발 세트',
                q: '쿠션 깔창', cat: '실내', pid: '7225268644', url: 'https://link.coupang.com/a/g3mRHER1mC',
                up: ['power_bank'],
                upT: '👟 쿠션 깔창 — 오래 서 있는 날 발바닥이 다릅니다' },
...
(function(){
  try{
    ...
    /* ⚠️ z-index 는 90 이다 — 축제·장소 모달(.fmodal)이 100 이라 그 «아래»에 있어야 한다.
       9998 로 두면 모달을 열었을 때 안내가 그 위로 떠오른다(2026-09-11 실측으로 잡음).
       ⚠️ bottom 은 safe-area 를 더한다 — 아이폰 홈 인디케이터·사파리 하단 바에 가리지 않게. */
    d.setAttribute('style','position:fixed;left:12px;right:12px;bottom:calc(12px + env(safe-area-inset-bottom,0px));z-index:90;background:#fff;border:1px solid #e5e0d8;border-radius:14px;box-shadow:0 6px 24px rgba(0,0,0,.18);padding:14px;font-size:14px;line-height:1.55;color:#2b2b2b');
...
// 🔔 2026-09-11 — 홈 상단 알림 신청 버튼.
//   왜: 구독 배너는 «첫 페이지뷰 10초 뒤에 저절로» 뜨는 방식뿐이라(OneSignal Push Slide Prompt),
//   그 10초 전에 나간 사람은 신청할 길이 아예 없었고, 이미 신청한 사람도 자기가 구독 중인지 알 수 없었다.
//   실측 근거(2026-09-11): 트래픽은 모바일 76%인데 구독자는 Win32 34 · 안드로이드 20 · iOS 0.
//   ⚠️ 기본이 hidden 이다. 웹푸시를 못 쓰는 브라우저(홈 화면 추가 전의 아이폰 사파리 등)에서는
//      끝까지 안 보인다 — 눌러도 아무 일 없는 죽은 버튼을 내보내지 않기 위해서다.
//   ⚠️ 정규식을 쓰지 않는다 — 템플릿 리터럴 안이라 \\/ 가 / 로 풀려 깨진다.
//   ⚠️ 브라우저가 «거부»로 기억한 경우엔 버튼으로 되돌릴 수 없다. 그때는 주소창 자물쇠에서
//      직접 바꿔야 한다고 «말해 준다» — 눌러도 안 되는 버튼을 남겨 두면 사람은 사이트를 탓한다.
const NOTI_JS = `<script>
(function(){
  var box=document.getElementById('home-noti');
  var btn=document.getElementById('noti-btn');
  var msg=document.getElementById('noti-state');
  if(!box||!btn||!msg) return;
  window.OneSignalDeferred=window.OneSignalDeferred||[];
  window.OneSignalDeferred.push(function(OneSignal){
    try{
      if(!OneSignal.Notifications||!OneSignal.Notifications.isPushSupported()) return;
      box.hidden=false;
      function paint(){
        var denied=(typeof Notification!=='undefined'&&Notification.permission==='denied');
        var on=!!(OneSignal.User&&OneSignal.User.PushSubscription&&OneSignal.User.PushSubscription.optedIn);
        if(denied){
          btn.textContent='🔕 알림이 차단되어 있어요';
          btn.disabled=true;btn.className='noti-btn off';
          msg.textContent='주소창 왼쪽 자물쇠 → 알림 → 허용으로 바꾸면 받을 수 있어요.';
          return;
        }
        btn.disabled=false;
        if(on){
          btn.textContent='🔔 알림 받는 중 — 끄기';
          btn.className='noti-btn on';
          msg.textContent='새 축제와 오늘 서는 장날을 알려드려요.';
        }else{
          btn.textContent='🔔 새 축제·장날 알림 받기';
          btn.className='noti-btn';
          msg.textContent='주말 근처 축제·행사 소식을 받아보세요. 언제든 끌 수 있어요.';
        }
      }
      paint();
      ...
      btn.addEventListener('click',function(){
        var on=!!(OneSignal.User&&OneSignal.User.PushSubscription&&OneSignal.User.PushSubscription.optedIn);
        if(window.gtag)gtag('event',on?'noti_off':'noti_on');
        var p=on?OneSignal.User.PushSubscription.optOut():OneSignal.User.PushSubscription.optIn();
        if(p&&p.then)p.then(function(){setTimeout(paint,400);}).catch(function(){setTimeout(paint,400);});
        else setTimeout(paint,800);
      });
    }catch(e){}
  });
})();
...
/* ⭐ 2026-09-03: h1 을 브랜드명으로 바꿨다. 같은 이름을 쓰는 travel-info.co.kr 이 구글 「축제모아」
   1위이고 우리는 2위인데, 그것도 홈이 아니라 /search 가 걸렸다 — 홈 h1 에 브랜드명이 없어서다.
   ⚠️ h1 을 숨기거나 1px 로 만들지 않는다(클로킹). 눈에 보이는 워드마크로 두고,
      원래 큰 문구는 바로 아래 부제로 내려 «보이는 것»은 사실상 그대로 유지한다. */
.home-left h1{font-size:clamp(1.35rem,2.2vw,1.7rem);line-height:1.2;letter-spacing:-.03em;margin:0 0 6px;color:#0a6c63;font-weight:900}
.home-h1sub{font-size:clamp(1.5rem,3vw,2.15rem);line-height:1.35;letter-spacing:-.02em;margin:0 0 10px;color:#1f2937;font-weight:800}
.home-h1sub b{color:#0a6c63}
.home-left>p{color:#6b7280;font-size:.94rem;margin:0 0 22px}
.home-left>p.home-h1sub{color:#1f2937;margin:0 0 10px}
...
/* 🔔 홈 알림 신청 — CTA 두 개보다 «한 단 낮은» 무게로 둔다. 여기가 주인공이 아니다. */
.home-noti{margin-top:14px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.noti-btn{background:#fff;color:#0c7d72;font-weight:800;font-size:.9rem;padding:10px 18px;border-radius:999px;border:1.5px solid #a9e5dd;cursor:pointer;transition:transform .15s}
.noti-btn:hover{transform:translateY(-2px)}
.noti-btn.on{background:#0f9d8f;border-color:#0f9d8f;color:#fff}
.noti-btn.off{background:#f3f4f6;border-color:#e5e7eb;color:#9ca3af;cursor:default}
.noti-btn.off:hover{transform:none}
.noti-state{font-size:.82rem;color:#5b6b68;line-height:1.5}
@media(max-width:640px){.home-noti{gap:7px}.noti-state{font-size:.79rem}}
...
.fav-mini{position:static;display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:50%;background:#f4f1ec;color:#9ca3af;border:1.5px solid #e5e7eb;font-size:1.05rem;box-shadow:none;margin-right:6px}
.fav-mini.on{color:#ff3b6b;background:#fff5f7;border-color:#ffc9d6}
...
<a href="/samcheok/">🦀 삼척</a>
<a href="/sokcho/">🏔 속초</a>
<a href="/chuncheon/">🚂 춘천</a>
<a href="/jeonju/">🏘 전주</a>
<a href="/gunsan/">🚢 군산</a>
...
function layout(title, desc, urlPath, content, opts) {
  ...
  // 🔁 2026-08-19 되돌림 — en/ja/zh noindex 는 판단 근거가 틀렸다.
  //   전날 「en 108imp/0clk · ja 61imp/0clk · zh 2imp/0clk」을 보고 전량 noindex 로 접었는데,
  //   그건 **구글 숫자만** 본 것이었다. noindex 는 구글에만 걸리지 않는다 — 빙·야후·네이버·
  //   덕덕고에도 똑같이 걸린다. GA4 28일 실측: /en/ 9세션 · /ja/ 6 · /zh/ 1 (합 16)이
  //   전부 Organic Search 로 들어오는데, 같은 기간 구글 클릭은 0이다.
  //   = 구글이 아닌 검색엔진이 유일하게 보내주던 유입을 우리 손으로 끊을 뻔했다.
  //   남겨둔 tw(4)+es(8)=12세션보다 오히려 많다.
  //   또 클릭 0의 원인은 «품질»이 아니라 «순위»다 — en 검색어 순위가 51~93위다(48위가 최고).
...
<a href="/ja/jangteo/">🏮 五日市</a><a href="/ja/places/">📍 行ってみる場所</a><a href="/ja/mountains/">⛰️ 名山</a><a href="/ja/cafe/">☕ カフェ</a></div></div>
<a href="/">🇰🇷 한국어</a></nav>`,
    es: `<nav>
<a href="/es/">Inicio</a>
...
/* 🔖 2026-09-11 — 관심사 태그. 알림을 «그 사람이 보러 온 것»으로 보내기 위해서다.
   근거(GA4 9/7~9/11, 구독자 54명이 붙은 나흘): 처음 들어온 페이지가
     월별 축제 41.8% · 오일장 31.3% · 홈 11.3% · 블로그 6.1% 로 갈린다.
   전체에게 같은 걸 보내면 둘 중 한쪽은 «내 얘기가 아니»라서 차단으로 간다.
   웹푸시는 한 번 차단당하면 되돌릴 방법이 없다 — 그래서 처음부터 갈라 보낸다.
   ⚠️ 무료 플랜 Data Tag 6개 한도 — interest·region·fest·month·lang·seen 이 정확히 6개다. 더 넣지 말 것.
   ⚠️ 정규식을 쓰지 않는다 — 이 코드는 템플릿 리터럴 안이라 \\/ 가 / 로 풀려 정규식이 깨진다. */
var TAG=(function(){try{
var p=location.pathname,t={},K="cjm_int";
var SIDO=["seoul","busan","daegu","incheon","gwangju","daejeon","ulsan","sejong","gyeonggi","gangwon","chungbuk","chungnam","jeonbuk","jeonnam","gyeongbuk","gyeongnam","jeju"];
var NAT=["trails","walk","mountain","valley","maple","flower","onsen","healing","cafe","pet","accessible","indoor","course","trip"];
var CITY=SIDO.concat(["gyeongju","gangneung","sokcho","jeonju","cheongju","suwon","yeosu","tongyeong","geoje"]);
var q=p.split("/"),a=q[1]||"",b=q[2]||"",cat="",reg="";
if(a==="jangteo"){cat="jangteo";if(SIDO.indexOf(b)>=0)reg=b;}
else if(a==="festival"){cat="festival";if(b)t.fest=b.slice(0,60);}
else if(a.length===7&&a.charAt(4)==="-"&&a.slice(0,2)==="20"){cat="festival";t.month=a;}
else if(a==="blog"){cat="blog";}
else if(NAT.indexOf(a)>=0){cat="nature";}
else if(CITY.indexOf(a)>=0){cat="city";reg=a;}
/* 마지막 한 페이지로 사람을 규정하지 않는다 — 방문을 누적해 «가장 많이 본 것»을 관심사로 삼는다 */
var c={};try{c=JSON.parse(localStorage.getItem(K)||"{}")||{};}catch(e){}
if(cat)c["c_"+cat]=(c["c_"+cat]||0)+1;
if(reg)c["r_"+reg]=(c["r_"+reg]||0)+1;
try{localStorage.setItem(K,JSON.stringify(c));}catch(e){}
var top=function(pre){var best="",n=0;for(var k in c){if(k.indexOf(pre)===0&&c[k]>n){n=c[k];best=k.slice(pre.length);}}return best;};
var bi=top("c_"),br=top("r_");
if(bi)t.interest=bi;
if(br)t.region=br;
t.lang=document.documentElement.lang||"ko";
t.seen=new Date().toISOString().slice(0,10);
return t;}catch(e){return null;}})();
window.OneSignalDeferred=window.OneSignalDeferred||[];window.OneSignalDeferred.push(function(OneSignal){OneSignal.init({appId:id}).then(function(){if(TAG&&OneSignal.User&&OneSignal.User.addTags)OneSignal.User.addTags(TAG);}).catch(function(){});});})();</script>
</head>
<body>
...
//     «월 조합»(서울전시회8월 14,680). 경쟁사가 노리는 상황형(홍대행사 200)은 수요가 없어 안 만든다.
const SEOUL_URLS = require('./seoul.js').build({ ROOT, layout, writePage, SITE, SITE_NAME, TODAY, WX });
// 🌏 2026-09-04 신설 — 외국어 도시 페이지 /{lang}/{city}/ (서울·부산·제주 × 5개어).
//   ⚠️ 재료 게이트가 안에 있다 — 장소 20곳·축제 15건 미만이면 «만들지 않는다».
//     중문·서어는 fetch-places.js 로 개요가 쌓이면 저절로 열린다.
...
// 🔴 2026-09-11 되돌림 — /pushtest/ 를 짓다가 «이미 OneSignal 이 깔려 있다»는 걸 뒤늦게 알았다.
//   위 2010행에 OneSignal SDK 가 전 페이지에 들어가 있고 루트에 OneSignalSDKWorker.js 도 있다.
//   내 서비스워커를 scope '/' 로 또 등록하면 그쪽 구독이 깨질 수 있어 통째로 뺀다.
//   ⚠️ 교훈: 새로 만들기 전에 «루트 파일 목록과 .vercelignore»를 먼저 읽을 것.
//      build.js 를 「구독·이메일·카카오」로만 찾아보고 "발송 시스템 없음"이라고 단정했는데,
//      정작 깔려 있던 건 그 단어가 하나도 안 들어가는 OneSignal 이었다.
...
//   ⭐ 도시 선정 근거: 방문자 수 ≠ 여행 수요(내국인 1위 강남구는 여행지가 아니다).
//     관광공사 방문 실측 + 네이버 여행 의도 검색량을 «같이» 봐서 외국인형·한국인형으로 나눴다.
const CITY_CORE = require('./city-core.js');
const { CITIES: CITY_CFGS } = require('./cities.js');
const CITY_URLS = [];
for (const cfg of CITY_CFGS) {
  const r = CITY_CORE.build(cfg, { ROOT, layout, writePage, SITE, SITE_NAME, TODAY, WX });
...
.jt-today-card .jt-rg{font-style:normal;font-weight:800;color:var(--jta,#15803d)}
...
// ⚠️ 2026-08-31 제목·설명 재설계 — 근거는 GSC 28일 실측(2026-08-03~08-30)이다.
//    · /jangteo/ 는 노출 2,501·클릭 36 = **CTR 1.4%** (평균순위 7.2). 순위는 있는데 클릭이 안 된다.
//    · 「오늘 장날」은 **평균 1.4위**인데 CTR 9.7%밖에 안 된다 → 그 말이 제목 끝에 있어 잘려 보인다.
//      옛 제목은 45자라 「— 오늘 열리는 장 바로 확인」이 구글에서 잘렸다. **앞으로 당긴다.**
//    · 끝자리 검색이 따로 큰 덩어리다: 「3 8장날」26 · 「3 8 오일장」17 · 「5일10일장날」15 ·
//      「2 7 오일장」12 · 「5 10 장날」12 · 「4 9 장날」11 = **93노출에 클릭 0**. 제목에 「끝자리」를 넣는다.
...
<p class="note" style="margin-top:18px">데이터 출처: 한국관광공사 TourAPI 전통시장 정보(판매 품목·장날·영업시간·주차·문의처)와 축제모아가 직접 정리한 유명 장터 자료. 장날은 <b>끝자리 간격이 5일 때만</b> 오일장으로 인정했습니다. 명절·기상에 따라 쉬는 날이 있으니 방문 전 확인하세요.</p>
    ...
    writePage('jangteo/' + SLUG[sido], layout(
      // 「경북 오늘 장날」33노출·클릭 0(순위 9.0)처럼 «지역 + 오늘 장날»이 실제로 검색된다.
      // 시·도 페이지 제목에도 그 말을 앞으로 뺀다(2026-08-31 GSC 실측).
      `${sido} 오늘 장날 — 오일장 ${withDay.length}곳 장날·영업시간 총정리 | ${SITE_NAME}`,
      `${sido} 오일장 ${withDay.length}곳의 장날을 끝자리별로 정리했습니다. ${Object.entries(endCnt).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([k, n]) => `${k}일장 ${n}곳`).join(' · ')}. 판매 품목과 영업시간·휴무·주차·문의처까지 한곳에.`,
...
(function(){
  ...
  // 킥④ 찜한 축제 D-day (메인)
  const __prevRenderFavs1 = window.renderFavs;
  window.renderFavs = function(){
    if (typeof __prevRenderFavs1 === 'function') __prevRenderFavs1();
...
const MARKET_FAV_JS = `<script>
(function(){
  window.renderFavs = window.renderFavs || function(){};
  var MS = 86400000;
  function opensOn(days, dt){ if (!days.length) return true; var ld = dt.getDate() % 10; for (var i=0;i<days.length;i++){ if ((days[i] % 10) === ld) return true; } return false; }
  function nextOffset(days, base){ if (!days.length) return 0; for (var o=0;o<5;o++){ if (opensOn(days, new Date(base.getTime()+o*MS))) return o; } return 0; }
  fetch('/market-fav.json').then(function(r){ return r.json(); }).then(function(M){
    var __prevRenderFavsM = window.renderFavs;
    window.renderFavs = function(){
      if (typeof __prevRenderFavsM === 'function') __prevRenderFavsM();
      var sec = document.getElementById('myjangteofavs-sec'), fbox = document.getElementById('myjangteofavs');
      if (!sec || !fbox) return;
      var favs = [];
      try { favs = JSON.parse(localStorage.getItem('cjm_favs')) || []; } catch(e) {}
      var mine = M.filter(function(m){ return favs.indexOf(m.n) !== -1; });
      if (!mine.length) { sec.style.display = 'none'; return; }
      sec.style.display = '';
      var today = new Date(); today.setHours(0,0,0,0);
      mine.forEach(function(m){ m.__off = nextOffset(m.dn || [], today); });
      mine.sort(function(a,b){ return a.__off - b.__off; });
      fbox.innerHTML = mine.map(function(m){
        var dd = m.__off === 0 ? '오늘 장날 🔥' : 'D+' + m.__off;
        var col = m.__off === 0 ? '#0f9d8f' : '#374151';
        return '<a class="wkchip" style="cursor:pointer" href="https://map.naver.com/p/search/' + encodeURIComponent(m.n) + '" target="_blank" rel="noopener">🏮 <strong>' + m.n +
          '</strong> <em style="font-style:normal;font-weight:800;color:' + col + '">' + dd + '</em><span>' + m.r + ' ' + m.c + ' · ' + (m.d || '') + '</span></a>';
      }).join('');
    };
    renderFavs();
  }).catch(function(){});
...
<div class="home-noti" id="home-noti" hidden><button type="button" id="noti-btn" class="noti-btn">🔔 새 축제·장날 알림 받기</button><span class="noti-state" id="noti-state"></span></div>
</div>
...
<div id="myjangteofavs-sec" style="display:none">
<h2 class="sec">🏮 내가 찜한 오일장</h2>
<div class="wkgrid" id="myjangteofavs"></div>
</div>
...
<h3>10가지 유형과 어울리는 축제</h3>
<p>테스트 없이 「나는 이 쪽이다」 싶은 유형이 있다면 바로 그 목록으로 가셔도 됩니다.</p>
<ul>
<li><b>💦 여름 물개상</b> — 물총·워터밤처럼 젖는 걸 각오하는 여름 축제. 더위가 한창일 때 몰려 있어 7~8월 일정을 먼저 보세요. 축제 뒤엔 <a href="/valley/">계곡</a>으로 이어 붙이면 하루가 채워집니다.</li>
<li><b>🎵 페스티벌 프로참석러</b> — 무대와 라인업이 중심인 음악 축제. 표가 미리 풀리는 편이라 <a href="/search/">축제 검색</a>에서 기간으로 걸러 미리 잡는 게 낫습니다.</li>
<li><b>🍜 축제는 먹으러 가는 편</b> — 지역 특산물이 주인공인 먹거리 축제. 같은 날 열리는 <a href="/jangteo/">오일장</a>을 끼우면 훨씬 잘 먹고 옵니다.</li>
<li><b>🌸 감성 사진 장인</b> — 벚꽃·유채·튤립 같은 <a href="/flower/">봄꽃 명소</a>와 가을 <a href="/maple/">단풍</a>. 며칠 차이로 절정이 지나가니 개화·절정 시기를 꼭 확인하세요.</li>
<li><b>🎭 문화 산책러</b> — 공연·전시·체험 위주라 붐빔이 덜한 평일에 가면 만족도가 올라갑니다. <a href="/trails/">걷기길</a>과 묶기 좋습니다.</li>
<li><b>🎆 불꽃 낭만러</b> — 하루 저녁에 몰리는 유형이라 자리싸움이 심합니다. 해 지는 시각과 교통 통제를 미리 보고 움직이세요.</li>
<li><b>🏮 옛멋 탐험가</b> — 민속·장터·전통놀이. <a href="/jangteo/">전국 오일장</a>이 사실상 같은 결이고, 장날만 맞추면 축제가 없는 주말에도 갈 곳이 생깁니다.</li>
<li><b>✨ 야경 수집가</b> — 빛축제·미디어아트. 겨울에 몰려 있어 <a href="/winter/">겨울 축제</a>와 겹칩니다.</li>
<li><b>⛄ 겨울왕국 주민</b> — 눈·얼음·빙어. 12~2월에 집중되고 결빙 상황에 따라 개막이 밀리는 일이 잦습니다.</li>
<li><b>🎪 축제 만능러</b> — 특별히 기우는 곳이 없는 유형. 이럴 땐 취향보다 <b>거리</b>로 고르는 게 실패가 적습니다. <a href="/">이번 주말 갈 만한 축제</a>부터 보세요.</li>
</ul>
<h3>자주 묻는 것</h3>
<p><b>결과에 나온 축제가 3개뿐인가요?</b><br>추천 카드는 조건에 가장 잘 맞는 3곳만 보여드립니다. 더 보시려면 <a href="/search/">축제 검색</a>에서 지역·기간·주제로 직접 좁혀 보세요.</p>
<p><b>이미 끝난 축제가 추천되기도 하나요?</b><br>아닙니다. 오늘 날짜 이후로 아직 끝나지 않은 축제만 후보에 넣습니다. 그래서 계절을 「눈 오는 겨울에」로 고르면 여름엔 후보가 적게 나올 수 있습니다.</p>
<p><b>추천이 안 나올 때도 있나요?</b><br>있습니다. 「당장 이번 주말」처럼 기간을 좁게 잡으면 그 주말에 열리는 축제가 몇 개 없을 수 있습니다. 없는 걸 있는 것처럼 채우지 않기 때문에, 그럴 땐 기간을 넓혀 다시 해보시는 게 빠릅니다.</p>
<p><b>같은 답을 골랐는데 결과가 달라졌어요.</b><br>유형은 그대로인데 <b>추천 축제만 바뀐 것</b>일 가능성이 큽니다. 축제 목록은 매일 갱신되고 끝난 축제는 빠지기 때문에, 어제 첫 번째로 뜨던 축제가 오늘은 안 뜰 수 있습니다. 마음에 든 축제가 있으면 그 자리에서 상세 페이지를 열어 두시는 게 좋습니다.</p>
<p><b>친구랑 결과를 맞춰 보고 싶어요.</b><br>결과 화면의 <b>공유하기</b>를 누르면 유형과 링크가 함께 복사됩니다. 유형이 서로 다르면 <b>둘 다 만족할 축제</b>를 찾는 게 관건인데, 이럴 땐 <a href="/course/">코스 짜기</a>에서 지역과 동행 조건만 넣어 보세요. 축제 하나에 걷기길·먹거리·카페를 붙여 반나절씩 나눠 담아 줍니다.</p>
</section>
${QUIZ_JS}
<h2 class="sec">월별 축제 일정 보기</h2>
${monthNavHtml}
...
<h3>축제에 갈 때는 계산이 한 번 더 뒤집힙니다</h3>
<p>평소 이동이라면 위 계산으로 충분한데, <b>축제는 사정이 다릅니다.</b> 금액에는 안 잡히지만 실제로는 크게 갈리는 것이 세 가지 있습니다.</p>
<ul>
<li><b>주차장이 축제장에서 멉니다.</b> 규모가 큰 축제는 임시주차장을 외곽에 두고 셔틀로 실어 나르는 경우가 많습니다. 계산기의 주차비 기본값이 0원인 이유도 이것 — 무료인 곳이 많은 대신 걷거나 셔틀을 타야 합니다. 유료 주차장을 쓸 생각이라면 <b>'계산 기준 바꾸기'</b>에서 주차비를 직접 넣어 보세요.</li>
<li><b>교통 통제가 걸립니다.</b> 불꽃축제·거리 퍼레이드처럼 하루 저녁에 몰리는 축제는 주변 도로를 막습니다. 이때는 자동차가 싸도 대중교통이 빠릅니다. 시간을 돈으로 환산하면 결과가 뒤집히는 구간입니다.</li>
<li><b>돌아오는 길이 더 막힙니다.</b> 폐막 시각에 사람이 한꺼번에 빠져나가기 때문에, 왕복을 단순히 두 배로 보면 실제보다 낙관적입니다.</li>
</ul>
<p>정리하면 — <b>사람이 적게 오는 축제·먼 거리·여럿이 간다</b>면 자동차, <b>도심 축제·혼자나 둘·저녁 축제</b>라면 대중교통 쪽이 대체로 낫습니다. 가려는 축제가 얼마나 붐비는지는 <a href="/hot/">요즘 어디 가지</a>에서 지역별 붐빔 배수로 확인하실 수 있습니다.</p>
<h3>자주 묻는 것</h3>
<p><b>왜 어떤 구간은 '요금 정보 없음'이라고 나오나요?</b><br>시외버스와 기차(KTX 등) 요금은 저희가 쓰는 대중교통 데이터에 들어 있지 않습니다. 없는 요금을 그럴듯하게 지어내지 않고 그대로 '없음'이라고 표시합니다. 이때는 코레일·시외버스 앱에서 확인하시는 게 정확합니다.</p>
<p><b>왕복 비용은 왜 안 보여주나요?</b><br>편도만 계산하고 두 배는 직접 하시게 두었습니다. 당일치기가 아니라 하루 자고 오거나, 갈 때만 차를 쓰는 경우가 흔해서 자동으로 두 배를 매기면 오히려 틀린 숫자가 되기 때문입니다.</p>
<p><b>전기차인데 어떻게 넣나요?</b><br>연비와 유가 칸을 각각 <b>전비(km/kWh)</b>와 <b>충전요금(원/kWh)</b>으로 바꿔 넣으시면 같은 식으로 계산됩니다. 공식 자체가 「거리 ÷ 효율 × 단가」라 단위만 맞추면 그대로 통합니다.</p>
</section>
...
// 🧹 2026-09-15 — robots 에서 sitemap.xml 줄을 뺐다.
//    실측: 섹션 6개 합집합 1,028개와 sitemap.xml 1,028개가 «차집합 0»인 완전 중복이었다.
//    두 줄을 다 두면 크롤러가 같은 1,028개를 두 번 읽는다. 지금 이 사이트는 허브 재크롤이
//    5~9일씩 밀리는 상태(2026-09-15: /ja/closed/ 9/6 · /jangteo/ 9/11)라 그 낭비가 아프다.
//    인덱스 한 줄이면 섹션 6개를 다 가리키므로 잃는 것이 없다.
// ⚠️ 그렇다고 sitemap.xml «파일»을 없애지는 말 것. /sitemap.xml 은 네이버·빙이 관습적으로
//    찾아가는 기본 경로다. 이 사이트는 세션의 70%가 네이버에서 온다 — 파일은 계속 만든다.
