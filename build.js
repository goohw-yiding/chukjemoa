...
function run(){
  ...
  fetch('/api/tripcost'+qs).then(function(r){return r.json();}).then(function(d){
    ...
    else if(d.intercity){var ic=d.intercity;html+='<div class="tc-box pt"><div class="mode">🚌 고속버스</div><div class="amt">'+won(ic.busEstimate)+'</div><div class="sub">거리 기반 추정 · <a href="'+ic.routeSearch+'" target="_blank" rel="noopener" style="color:#0c6d9c;font-weight:700">정확히 보기 🔎</a></div></div>';}
    ...
    else if(d.intercity){var df=d.diff;if(df>0)html+='<div class="tc-verdict">🚌 고속버스(추정)가 '+won(df)+' 더 저렴해요</div>';else if(df<0)html+='<div class="tc-verdict">🚗 자동차가 '+won(-df)+' 더 저렴해요 (추정 대비)</div>';}
    html+='</div>';
    out.innerHTML=html;
