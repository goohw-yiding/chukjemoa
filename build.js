...
(function(){
  ...
  window.cjmResetCalc=function(p,dest,alt){
    window.cjmDest[p]={d:dest||'',a:alt||''};
  ...
  window.cjmBindCalc=function(p){
    ...
    function ask(from,to,onFail){
      return fetch('/api/tripcost?from='+encodeURIComponent(from)+'&to='+encodeURIComponent(to))
        .then(function(r){return r.json();})
        .then(function(d){
          if(d.error){ if(onFail){onFail(d.error);return;} done();o.innerHTML='<div class="err">'+esc(d.error)+'</div>';return; }
          done();render(d);
        });
    }
    function run(){
      var from=i.value.trim(),t=window.cjmDest[p]||{};
      if(!from){o.innerHTML='<div class="err">출발지를 입력해주세요.</div>';i.focus();return;}
      if(!t.d){o.innerHTML='<div class="err">도착지를 확인할 수 없어요.</div>';return;}
      try{localStorage.setItem(KEY,from);}catch(e){}
      b.disabled=true;b.textContent='계산 중…';o.innerHTML='';
      ask(from,t.d,function(err){
        if(t.a&&t.a!==t.d){ ask(from,t.a).catch(function(){done();o.innerHTML='<div class="err">계산에 실패했어요. 잠시 후 다시 시도해주세요.</div>';}); }
        else{done();o.innerHTML='<div class="err">'+esc(err)+'</div>';}
      }).catch(function(){done();o.innerHTML='<div class="err">계산에 실패했어요. 잠시 후 다시 시도해주세요.</div>';});
    }
...
(function(){
  ...
  window.openFestModal=function(ds){
    ...
    if(window.cjmResetCalc)window.cjmResetCalc('fmc',ds.name||ds.place||'',[ds.city,ds.place].filter(Boolean).join(' ')||ds.place||'');
...
(function(){
  ...
  window.openPlaceModal=function(o){
    ...
    if(window.cjmResetCalc)window.cjmResetCalc('pmc',o.title||'',o.dest||'');
    m.classList.add('show');
  };
  function close(){m.classList.remove('show');}
