...
function run(){
  ...
  fetch('/api/tripcost'+qs).then(function(r){return r.json();}).then(function(d){
    ...
    else if(d.intercity){var ic=d.intercity;html+='<div class="tc-box pt"><div class="mode">🚌 '+(ic.busLabel||'대중교통')+'</div><div class="amt">'+won(ic.busEstimate)+'</div><div class="sub">거리 기반 추정 · <a href="'+ic.routeSearch+'" target="_blank" rel="noopener" style="color:#0c6d9c;font-weight:700">정확히 보기 🔎</a></div></div>';}
    ...
    html+='</div>';
    html+='<div class="tc-break"><b>🧮 이렇게 계산했어요</b><br>· 자동차: '+car.formula+'<br>· 기준: 연비 '+d.assumptions.kmpl+'km/L, 유가 '+won(d.assumptions.fuelPrice)+'/L'+(d.assumptions.parking?', 주차 '+won(d.assumptions.parking):'')+((pt&&pt.fare)?'<br>· 대중교통: ODsay 최적경로 요금 '+won(pt.fare):(d.intercity?'<br>· 고속버스: 거리 추정(약 90원/km) — 시외버스·기차는 공개 요금 API가 없어 추정값이에요':''))+'</div>';
    ...
    else if(d.intercity){var df=d.diff;if(df>0)html+='<div class="tc-verdict">🚌 '+(d.intercity.busLabel||'대중교통')+'(추정)이 '+won(df)+' 더 저렴해요</div>';else if(df<0)html+='<div class="tc-verdict">🚗 자동차가 '+won(-df)+' 더 저렴해요 (추정 대비)</div>';}
    html+='</div>';
    out.innerHTML=html;
