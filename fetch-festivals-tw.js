...
// ⚠️ 번체는 인증키가 따로다(ChtService2를 별도 활용신청하며 받은 키). tourapi-tw.key 우선.
const KEY=(function(){
...
async function main(){
  ...
  while((page-1)*rows<total){
    ...
    let j; try{j=JSON.parse(txt);}catch(e){console.error('parse err',txt.slice(0,200));break;}
    const b=j.response&&j.response.body; if(!b){console.error('no body',txt.slice(0,200));break;}
  ...
  const fpath=path.join(__dirname,'data','festivals_tw.json'); const cache={};
  ...
  console.log('번체 축제 저장:', out.length, '| 개요:', out.filter(f=>f.ov).length, '| 지역:', JSON.stringify(byr));
}
main();
