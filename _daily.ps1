...
W "3) mirror + drift check"
& node _d2p.js build.js festival.js data/fest_trend.json 2>&1 | Select-Object -Last 1 | ForEach-Object { W "   $_" }
$sync = (& node _sync.js 2>&1 | Select-Object -Last 1)
W "   $sync"

$changed = @(& git status --porcelain).Count
if ($changed -eq 0) { W "4) no changes - skip deploy"; W "===== END ====="; exit 0 }
W "4) deploy ($changed changed)"
& git add -A
