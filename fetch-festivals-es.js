// 서어(스페인어) 축제(SpnService2 searchFestival2 + detailCommon2) → data/festivals_es.json
const fs=require('fs'), path=require('path'), https=require('https');
const KEY=fs.readFileSync(path.join(__dirname,'tourapi.key'),'utf8').trim();
const BASE='https://apis.data.go.kr/B551011/SpnService2';
...
async function main(){
  ...
  const fpath=path.join(__dirname,'data','festivals_es.json'); const cache={};
  ...
  console.log('스페인어 축제 저장:', out.length, '| 개요:', out.filter(f=>f.ov).length, '| 지역:', JSON.stringify(byr));
}
main();
