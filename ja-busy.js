...
${/* 🇰🇷 코리 블록으로 가는 «다리» 한 줄.
     ⚠️ 상품 링크를 본문에 넣지 않는다(대표님 방침: 상품을 인위적으로 앞에 빼지 않는다).
        코리 블록은 build.js 의 writePage 가 맨 아래에 «자동으로» 붙인다 — 여기서는
        그 블록 바로 위에 «이 페이지의 논리 안에 있는» 한 문장만 둔다.
     왜 이 문장인가: 이 페이지가 내내 「그날 가게가 닫힌다」를 말했다. 그러면 다음 질문이
        「おみやげはいつ買うのか」다. 억지로 끌어온 이야기가 아니라 이 페이지가 만든 질문이다. */''}
<p style="color:#57534e;font-size:.93rem;line-height:1.8;margin:22px 0 0">🎁 <b>おみやげを最終日にまとめて買う予定なら、その日が上の表のどれかに当たっていないか見ておいてください。</b>
名節は個人商店も休みますし、${WD[worstW]}曜は定休日の店が${nf(cntW(worstW))}店あります。デパートや空港は開いていますが、市場や路面の店は閉まります。</p>
</div></main>`;
