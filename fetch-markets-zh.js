// 🏮 전국 오일장·전통시장 중문 간체판 — TourAPI ChsService2 쇼핑(79) cat3=A04010100(오일장)+A04010200(전통시장) → data/markets_zh.json
// ※ 영문판(fetch-markets-en.js)과 같은 구조. contentTypeId=79 는 EngService2 전용이 아니라
//   Jpn/Spn/Chs 언어서비스에도 동일하게 존재한다(2026-08-27 실측).
//   region 은 1차로 lDongRegnCd(행정구역코드)를 RMAP으로 매핑한다 — fetch-festivals-zh.js 와 동일 코드.
//   드물게 코드가 비어 오는 항목은 addr1 텍스트에서 지역명을 찾아 보완한다.
const fs = require('fs'), path = require('path'), https = require('https');
const KEY = fs.readFileSync(path.join(__dirname, 'tourapi.key'), 'utf8').trim();
const B = 'https://apis.data.go.kr/B551011/ChsService2';
...
async function run() {
  ...
  fs.writeFileSync(path.join(__dirname, 'data', 'markets_zh.json'), JSON.stringify(out));
  console.log('✓ data/markets_zh.json 저장');
}
